...
export const getMetricsForGraph = async (req, res) => {
    try {
      ...
      const result = await conn.execute(`select ${key}, record_dt from user_metrics where user_id_fk='${userid}'
  ...
  export const addMetricsToHist = async (req, res) => {
    ...
    try {
        ...
        date = new Date();
      conn = await connect();
      let {metricid, userid, bodyweight, bicep, hip, waist, chest, nutrition, xexperience} = req.body; //key can be body_weight, bicep_measurement, hit_measurement, waist_measurement, chest_measurement
      ...
      {metid: metricid, useridfk: userid, currdt: date, weigh: bodyweight, bicepm: bicep, hipm: hip, waistm: waist, chestm: chest, nutidfk: nutrition, explevel: xexperience},
      { autoCommit: true }
      );
